...
def test_list_bottles_displays_non_vintage_wine(tmp_path: Path) -> None:
    input_path = tmp_path / "cave.csv"
    database_path = tmp_path / "cellarmind.sqlite"
    ...
    assert bottles[0].producer == "Maison Test"
    assert bottles[0].cuvee == "Brut Réserve"
    assert bottles[0].vintage == "NV"
